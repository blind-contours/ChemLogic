...
receptor_1_SVD_grps_results <- plot_SVD_kmeans(receptor_type = "Androgen", 
                ...
                k = 4, ## number of k-means clusters from plot above
...
receptor_1_logic_MCMC_results <- run_logic_MCMC(
  outcome_label = quo(Label_names),
  filter_type = "Pubchem",
  binary_data = receptor_1_data_wPCs_kgrps, #make sure to update your binary data to be the dataframe with PCs and k-means groups
  subgroups = TRUE,
...
map(.x = tables, .f = matt_coef)
```

```{r }

tree_var_w_names <- AR_data_PCs_grps %>% 
  select(Name, contains(filter_type),
         Label_names)


full_data <- merge(AR_rdkit_descriptions, tree_var_w_names, 
                                   by.x = "NAME",
                                   by.y = "Name")

AR_data_PCs_grps_tree_var_cln <- full_data %>% 
  select(-c(NAME,SMILES,LABEL,mol, X))
